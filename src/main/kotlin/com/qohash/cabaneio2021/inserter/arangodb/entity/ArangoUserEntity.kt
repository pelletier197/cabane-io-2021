...
abstract class ArangoUserEntity(
    ...
    @Relations(
        edges = [PostedRelation::class],
        minDepth = 3,
        maxDepth = 3,
        direction = Relations.Direction.OUTBOUND
    ) open var publications: List<ArangoPublicationEntity>,
    @Relations(
        edges = [FollowsRelation::class],
        direction = Relations.Direction.OUTBOUND
    ) open var follows: List<ArangoUserEntity>,
    @Relations(
        edges = [LikesRelation::class],
        minDepth = 3,
        maxDepth = 3,
        direction = Relations.Direction.OUTBOUND
    ) open var likes: List<ArangoTweetEntity>
) {
